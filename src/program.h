...
{
    uint64_t result = 0;
    __asm__ volatile ("movq $3, %%rdi; movq %1, %%rsi; movq %2, %%rdx; movq %3, %%rcx; int $0x80; movq %%rax, %0" : "=r"(result) : "r"((uint64_t)width), "r"((uint64_t)height), "r"(title) : "%rdi", "%rsi", "%rdx", "%rcx");
...
{
    uint64_t result = 0;
    __asm__ volatile ("movq $7, %%rdi; movq %1, %%rsi; movq %2, %%rdx; movq %3, %%rcx; int $0x80; movq %%rax, %0" : "=r"(result) : "r"((uint64_t)name), "r"((uint64_t)binary), "r"((uint64_t)size) : "%rdi", "%rsi", "%rdx", "%rcx");
